...
const Modal = (props) => {
    ...
    const checkVideoInPlaylist = (videoID, playlistID) => {
        const currentPlaylist = playlistState.library.find(playlist => playlist._id === playlistID)
    ...
    const createPlaylistHandler = async () => {
        setPlaylistName('')
        const response = await networkCall(`/playlist/${props.videoID}`, "POST", { name: playlistName })
        if (response.status === 200) {
            playlistDispatch({ type: 'ADD_NEW_PLAYLIST', payload: response.data.playlist })
            dispatch({ type: 'SET_SHOW_MODAL', payload: '' })
        }
    }
        ...
        <div className='modal'>
            <div className='modal-container'>
                ...
                <div className="playlists-container">
                    <div className="title"><h2>Playlists</h2></div>
                    ...
                    {createNewPlaylist
                        ? <>
                            <div className='playlist-name-input'>
                                <input type="text" placeholder='Name of playlist' ref={textInputRef} onChange={(e) => setPlaylistName(e.target.value)} value={playlistName} />
                            </div>
                            <div className='playlist-create-btn'>
                                <button disabled={playlistName.length === 0 && 'disabled'} onClick={() => createPlaylistHandler()}>Create</button>
                            </div>
                        </>
                        : <>
                            <button className='create-playlist-btn' onClick={() => expandModalHandler()}
                            ><AiOutlinePlus /> Create playlist</button>
                        </>
                    }
